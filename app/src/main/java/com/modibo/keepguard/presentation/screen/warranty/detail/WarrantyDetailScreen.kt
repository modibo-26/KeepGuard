    ...
    Scaffold(
                ...
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(16.dp)) {
                        if (warranty.provider.isNotEmpty()) {
                            DetailRow("Fournisseur", warranty.provider)
                        }
                        DetailRow("Début", dateFormat.format(Date(warranty.startDate)))
                        DetailRow("Fin", dateFormat.format(Date(warranty.endDate)))
                        DetailRow("Durée", "${warranty.durationMonths} mois")
                        if (warranty.conditions.isNotEmpty()) {
                            DetailRow("Conditions", warranty.conditions)
                        }
                    }
                }
            }
        ...
        if (showDeleteDialog) {
            AlertDialog(
                ...
                {
                    TextButton(onClick = { viewModel.deleteWarranty() }) {
                        Text("Supprimer")
                    }
                },
                dismissButton = {
                ...
                text = { Text("Voulez-vous vraiment supprimer cette garantie ?") },
            )
        }
    }
}
